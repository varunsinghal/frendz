...
class MessageModel{
	...
	public function fetchMessageThreadsFor($user_id){
		$sql = "
		SELECT message_detail.message_id, message_detail.message_content, message_calc.other_user, frendz_user.user_first_name, frendz_user.user_last_name, message_detail.from_user_id, frendz_user.user_email, message_detail.created_on FROM message_detail INNER JOIN (
				SELECT
			...
			INNER JOIN frendz_user ON frendz_user.user_id = message_calc.other_user order by message_detail.created_on desc";
	...
	public function fetchConversation($from, $to){
		$sql = "SELECT A.*, frendz_user.user_first_name, frendz_user.user_last_name, frendz_user.user_email from (SELECT message_id, message_content, from_user_id, created_on from message_detail where (from_user_id=:from_id and to_user_id=:to_id) or (from_user_id=:to_id and to_user_id=:from_id)) A left join frendz_user on A.from_user_id = frendz_user.user_id order by A.created_on asc";
		$query = $this->db->prepare($sql);
