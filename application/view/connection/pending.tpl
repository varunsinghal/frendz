...
<table class="connectiontable">
	...
	<tr>
		...
		<td>
			<a href="{URL}connection/id/{$thread->user_id}">{$thread->user_first_name} {$thread->user_last_name}</a><br/>

			<a href="{URL}connection/accept/{$thread->user_id}"><i class="fa fa-check" aria-hidden="true"></i>  Accept</a> &nbsp;&nbsp; &nbsp;&nbsp; <a href="{URL}connection/decline/{$thread->user_id}"><i class="fa fa-times" aria-hidden="true"></i> Decline</a>
			{if $thread->note|default:FALSE}
			<br/><i class="fa fa-sticky-note-o" aria-hidden="true"></i> {$thread->note}
			{/if}
			<br/>request received {$thread->created_on|relativedate}
